...
public class Auto {
  ...
  public static NumericScale makeNumericScale(NumericExtentDetail extentDetail, boolean nice, double[] padFraction, double includeZeroTolerance, int desiredTickCount, boolean forBinning) {
    ...
    if (desiredTickCount < 1) {
      desiredTickCount = Math.min(extentDetail.optimalBinCount, 20) + 1;
    }
  ...
  private static String transformForSkew(Double skew, Double min, Double max) {
    if (skew == null) {
      return "linear";
    } else if (skew > 2 && min > 0 && max > 75 * min) {
      return "log";
    } else if (skew > 1.0 && min >= 0) {
      return "root";
    } else {
      return "linear";
    }
  ...
  public static int optimalBinCount(Field f) {
    ...
    if (stddev == null) {
      return 2;                // Guard against single-valued data
    }
  ...
  public static Field convert(Field base) {
    if (base.isSynthetic() || base.isDate()) {
      return base;           // Already set
    }
    if (base.isProperty("list")) {
      return base;                     // Already a multi-set
    }
    ...
    if (base.isNumeric()) {
      ...
      while (n < N && n < 50) {
        ...
        n++;
        if (!(o instanceof Date) && Data.asNumeric(o) != null) {
          nNumeric++;
        }
      }
  ...
  private static boolean goodLists(Field f) {
    int nValid = f.valid();
    if (nValid < 3) {
      return false;                                   // Too few to autoconvert
    }
  ...
  private static boolean isYearly(Field asNumeric) {
    if (asNumeric.numProperty("q1") < 1600) {
      return false;           // Use the lower quartile (to avoid outliers)
    }
    if (asNumeric.numProperty("q3") > 2100) {
      return false;           // High value is usually OK
    }
  ...
  /**
   * Make a nice simple linear scale.
   * Intended for external use
   *
   * @param low  scale low value
   * @param high scale high value
   * @return a nice range for the scale
   */
  public static Range makeSimpleLinearScale(double low, double high) {
    NumericExtentDetail extent = NumericExtentDetail.makeForSimpleRange(low, high, null, false);
    NumericScale scale = makeNumericScale(extent, true, new double[]{0, 0}, 0.2, -1, false);
    return Range.make(scale.min, scale.max);
  }
}
