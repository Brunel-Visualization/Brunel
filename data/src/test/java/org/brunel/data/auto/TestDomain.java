...
public class TestDomain {
	...
	public void testOneCategorical() {
		...
		Object[] content = domain.span(0).content();
		assertEquals(3, content.length);
		assertEquals("a", content[0]);
		assertEquals("c", content[1]);
		assertEquals("f", content[2]);

		assertEquals(2, domain.range(0).length);
		assertEquals(0.0, domain.range(0)[0], 1e-9);
		assertEquals(1.0, domain.range(0)[1], 1e-9);
	}
	...
	public void testTwoNumeric() {
		...
		assertEquals(2, domain.span(0).content().length);
		assertEquals(3.0, domain.span(0).content()[0]);
		assertEquals(50.0, domain.span(0).content()[1]);
	...
	public void testMixedDomains() {
		...
		// First span: time
		Object[] content1 = domain.span(0).content();
		...
		// First numeric (date)
		assertEquals(0.0, domain.range(0)[0], 1e-9);
		assertEquals(R, domain.range(0)[1], 1e-9);

		// Second numeric
		assertEquals(1.1 * R, domain.range(1)[0], 1e-9);
		assertEquals(2.1 * R, domain.range(1)[1], 1e-9);

		// Third: categorical
		assertEquals(2.2 * R, domain.range(2)[0], 1e-9);
		assertEquals(1.0, domain.range(2)[1], 1e-9);
	}
	...
	public void testBinnedAlone() {
		...
		assertEquals(1, domain.spanCount());
		assertEquals(3, domain.span(0).content().length);
		assertEquals("0\u202620", Data.format(domain.span(0).content()[0], false));
		assertEquals("20\u202640", Data.format(domain.span(0).content()[1], false));
		assertEquals("40\u202660", Data.format(domain.span(0).content()[2], false));

		domain = new Domain(true).include(binnedA);
		assertEquals(1, domain.spanCount());
		assertEquals(2, domain.span(0).content().length);
		assertEquals(0.0, domain.span(0).content()[0]);
		assertEquals(60.0, domain.span(0).content()[1]);
	...
	public void testBinned() {
		...
		assertEquals(1, domain.spanCount());
		assertEquals(6, domain.span(0).content().length);
		assertEquals("0\u202620", Data.format(domain.span(0).content()[0], false));
		assertEquals("20\u202640", Data.format(domain.span(0).content()[1], false));
		assertEquals("40\u202660", Data.format(domain.span(0).content()[2], false));
		assertEquals("a", domain.span(0).content()[3]);
		assertEquals("c", domain.span(0).content()[4]);
		assertEquals("f", domain.span(0).content()[5]);

		// Binned with numeric data -- one set of numeric range
		domain = new Domain(true).include(b).include(binnedA);
		assertEquals(1, domain.spanCount());
		assertEquals(2, domain.span(0).content().length);
		assertEquals(-45.0, domain.span(0).content()[0]);
		assertEquals(60.0, domain.span(0).content()[1]);
