...
public class Parser {
					...
					if (expectParameter) {
						...
						if (":".equals(token.content)) {
							...
							if (nextToken.content.equals("[")) {
								...
								i += parseList(tokens.subList(i + 1, parametersEnd + 1), listContent);
	...
	/**
	 * Work through tokens and add them into the list, returning the index of the ']' ending the list
	 *
	 * @param tokens      list of tokens to look through
	 * @param listContent the built list of items
	 * @return the number of tokens read from the list, including the last ']'
	 */
	private int parseList(List<BrunelToken> tokens, List<Param> listContent) {
		// The tokens should look like VAL , VAL , ..., ] potentially with some missing values

		boolean expectValue = true;
		for (int i = 0; i < tokens.size(); i++) {
			BrunelToken tok = tokens.get(i);
			if (tok.content.equals("]")) {
				// Done -- return how much we have read
				if (i>0 && expectValue) listContent.add(null);        // We have a missing value before the end
				tok.parsedType = "syntax";
				return i + 1;
			} else if (tok.content.equals(",")) {
				if (expectValue) listContent.add(null);        // We have a missing value
				tok.parsedType = "syntax";
				expectValue = true;                            // After a comma, expect a value
			} else {
				listContent.add(parseModifier(tok));        // A new value
				expectValue = false;                        // After a value, do not expect another
			}

		}
		throw new IllegalStateException("Badly defined list");
	}
