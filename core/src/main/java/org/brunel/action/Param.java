...
public class Param implements Comparable<Param> {
  ...
  public double asDouble() {
    if (type == Type.number) {
      ...
      return Data.parseDouble(content.toString());
    }
  }
  ...
  public int asInteger() {
    if (type == Type.number) {
      ...
      return Data.parseInt(content.toString());
    }
  }
  ...
  public boolean hasModifierOption(String optionName) {
    for (Param p : modifiers) {
      if (p.asString().equalsIgnoreCase(optionName)) {
        return true;
      }
    }
    return false;
