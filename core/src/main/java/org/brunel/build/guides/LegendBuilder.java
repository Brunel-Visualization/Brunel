...
public class LegendBuilder {
  ...
  private final boolean isFloating;           // does the legend float?
  ...
  public void positionFloatingLegend() {
    out.onNewLine()
      .add("var legendBox = legends.node().getBBox(), lw = legendBox.width, lh = legendBox.height, ")
      .onNewLine().add("L = geom.inner_left+5, T = geom.inner_top+5, W = geom.inner_width-10, H = geom.inner_height-10")
      .endStatement();
    out.add("function box(a,b) { return { x:L+(W-lw)*a, y:T+(H-lh)*b, width:lw, height:lh } }").endStatement();
    out.add("var locs = [ box(1,0), box(1,1), box(1, 0.5), box(0,1), box(0, 0.5), box(0,0) ]").endStatement();
    out.add("var B = BrunelD3.lowestBoxHits(locs, elements)").endStatement();
    out.add("legends.transition().duration(200).ease(d3.easeLinear)")
      .addChained("attr('transform', 'translate(' + (B.x + lw) + ',' + B.y + ')' )")
      .endStatement();
  }

  private boolean getFloating() {
    for (ElementStructure e : structure.elementStructure) {
      VisElement vis = e.vis;
      if (vis.tLegends != Legends.none) {
        return vis.tFloatingLegend;
      }
    }
    return false;
  }
  ...
  public boolean isFloating() {
    return needsLegends() && isFloating;
  }
  ...
  public void defineUsageForLegend(ElementStructure e) {
    ...
    if (defineColor || defineSymbol) {
      ...
      if (defineColor) {
        out.add("legends._legend.color = scale_color").endStatement();
      }
      if (defineSymbol) {
        out.add("legends._legend.symbol = scale_symbol").endStatement();
      }
    }
  }

  private int legendWidth() {
    if (!needsLegends() || isFloating) {
      return 0;
    }
  ...
  public void writeLegends() {
    if (needsLegends()) {
      out.onNewLine().add("BrunelD3.addLegend(legends, legends._legend)").endStatement();
    }
  }
  ...
  private Field getTargetField(String aestheticName, Field requiredSameAs) {
    ...
    for (ElementStructure e : structure.elementStructure) {
      ...
      if (aesthetic == null) {
        continue;                            // No aesthetic means no  legend
      }
      if (auto && aesthetic.name.equals("#selection")) {
        continue;  // No automatic legend for selection
      }

      if (aestheticName.equals(vis.tLegends.name())) {
        return aesthetic;  // Specific request
      }
    ...
    if (field.preferCategorical()) {
      ...
      if (details.granular) {
        ...
        for (int i = 0; i < newDiv.length; i++) {
          newDiv[i] = (divisions[i] + divisions[i + 1]) / 2;
        }
        divisions = newDiv;
      }
      // Reverse
  ...
  private boolean same(Field a, Field b) {
    ...
    if (a.preferCategorical()) {
      ...
      return (Math.abs(a.min() - b.min()) + Math.abs(a.max() - b.max()) > 1e-6);
    }
  }
