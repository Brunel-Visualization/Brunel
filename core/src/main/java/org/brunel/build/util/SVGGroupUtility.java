...
public class SVGGroupUtility {
  ...
  public void defineParallelAxisClipPath(int width) {
    // Add a clip path for a parellel axis. Note that widths are defined later
    out.add("vis.select('defs').append('clipPath').attr('id', '" + clipID("parallel") + "').append('rect')")
      .addChained("attr('y', 0).attr('height', geom.inner_rawHeight+1)")
      .endStatement();
  }
  ...
  public String clipID(String part) {
