...
public class InteractionDetails {
  ...
  public static Param getInteractionParam(VisElement vis, Interaction type) {
    for (Param p : vis.tInteraction) {
      if (p.asEnum(Interaction.class) == type) {
        return p;
      }
    }
    return null;
  ...
  public InteractionDetails(VisTypes.Diagram diagram, ChartCoordinates coordinates, VisElement[] elements) {
    ...
    if (zoomTypes == null) {
      zoomTypes = defaultZooms();                  // defaults if no user request
    }
    ...
    userZoomBounds = zoomBounds(elements);
  ...
  public void addHandlers(ElementStructure structure, ScriptWriter out) {
    ...
    for (Param p : interactions) {
      ...
      for (Param mod : p.modifiers()) {
        if (mod.toString().startsWith("snap")) {
          isSnap = true;
        }
      }
  ...
  public void addOverlayForZoom(VisTypes.Diagram diagram, ScriptWriter out) {
    ...
    String extent = makeZoomMinMaxRange(diagram, userZoomBounds);
  ...
  private String makeZoomMinMaxRange(VisTypes.Diagram diagram, double[] userZoomBounds) {
    if (userZoomBounds != null) {
      double min = Math.min(userZoomBounds[0], userZoomBounds[1]);
      double max = Math.max(userZoomBounds[0], userZoomBounds[1]);
      return min + "," + max;
    } else if (diagram == VisTypes.Diagram.map) {
      return "1/5,5";
    } else if (diagram == VisTypes.Diagram.parallel ||
      diagram == VisTypes.Diagram.dag || diagram == VisTypes.Diagram.tree || diagram == VisTypes.Diagram.network) {
      return "1/2,10";
    } else {
      return "1/3,3";
    }
  ...
  public void addZoomFunctionality(ScriptWriter out) {
    ...
    if (diagram == VisTypes.Diagram.parallel) {           // Very special handling
      applyParallelAxisZoom(out);
    } else if (diagram != VisTypes.Diagram.map) {         // The map has no scales to modify ...
      if (canZoomX) {
        applyZoomToScale(0, out);
      }
      if (canZoomY) {
        applyZoomToScale(1, out);
      }
    }
  ...
  protected void applyParallelAxisZoom(ScriptWriter out) {
  ...
  public boolean hasElementInteraction(ElementStructure structure) {
    if (!structure.vis.itemsTooltip.isEmpty()) {
      return true;                 // tooltips require a handler
    }
    if (structure.chart.diagram == VisTypes.Diagram.network) {
      return true;   // networks are draggable
    }
    if (structure.chart.diagram == VisTypes.Diagram.parallel) {
      return true;   // parallel axes use them by default
    }
  ...
  private boolean banned(VisElement[] elements, Interaction type) {
    ...
    for (VisElement vis : elements) {
      if (getInteractionParam(vis, Interaction.none) != null) {
        return true;
      }
    }
  ...
  private boolean[] defaultZooms() {
    ...
    if (diagram == VisTypes.Diagram.network || diagram == VisTypes.Diagram.map || diagram == VisTypes.Diagram.dag
      || diagram == VisTypes.Diagram.tree) {
      return ZOOM_ALL;
    }

    // Parallel coordinates gets special zooming -- just the "Y"
    if (diagram == VisTypes.Diagram.parallel) {
  ...
  private boolean expandRequested(VisElement[] elements) {
    ...
    for (VisElement vis : elements) {
      if (getInteractionParam(vis, Interaction.expand) != null) {
        return true;
      }
    }
    ...
    for (Param p : param.modifiers()) {
      if (p.asString().startsWith("snap")) {
        if (p.asString().equalsIgnoreCase("snapx")) {
          snapFunction = "x";
        } else if (p.asString().equalsIgnoreCase("snapy")) {
          snapFunction = "y";
        } else {
          snapFunction = "xy";
        }
  ...
  private boolean targetsElement(Param param) {
    if (param == null) {
      return false;                            // No interaction => no targeting
    }
    for (Param p : param.modifiers()) {
      if (p.asString().startsWith("snap")) {
        return false;      // snap means the handler is attached to background
      }
    }
  ...
  // If the user has specified bounds for a zoom, return them
  private double[] zoomBounds(VisElement[] elements) {
    for (VisElement vis : elements) {
      Param param = getInteractionParam(vis, Interaction.panzoom);
      if (param != null) {
        try {
          List<Param> list = param.firstListModifier();
          if (list != null) {
            if (list.size() == 1) {
              double v = Math.max(1e-6, list.get(0).asDouble());
              return new double[]{1 / v, v};
            }
            if (list.size() == 2) {
              return new double[]{Math.max(1e-6, list.get(0).asDouble()), Math.max(1e-6, list.get(1).asDouble())};
            }

          }
        } catch (NumberFormatException e) {
          throw new IllegalArgumentException("The arguments for pan zoom bounds must be a list of exactly two numeric values");
        }
      }
    }
    return null;
