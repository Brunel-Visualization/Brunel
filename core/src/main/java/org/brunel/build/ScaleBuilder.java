...
public class ScaleBuilder {
  ...
  public int defineScaleWithDomain(String name, Field[] fields, ScalePurpose purpose, int numericDomainDivs,
    ...
    for (Field field : fields) {
      domain.include(field);
    }

    // Easy case -- no domains at all
    if (domain.spanCount() == 0) {
    ...
    if (field.isDate()) {
      ...
      if (purpose == ScalePurpose.x) {
        dateFormat = coordinates.xDateFormat;
      }
      if (purpose == ScalePurpose.y) {
        dateFormat = coordinates.yDateFormat;
      }
      ...
      if (purpose == ScalePurpose.sizeAesthetic) {
    ...
    String domainDivs = Data.join(divs, null, false);
  ...
  public boolean elementsFillHorizontal(ScalePurpose purpose) {
    for (VisElement e : elements) {
      ...
      if (e.tElement != Element.line && e.tElement != Element.area) {
        return false;
      }
      // There must be no clustering on the X axis
      if (purpose == ScalePurpose.x && e.fX.size() > 1) {
        return false;
      }
    }
  ...
  public Double getGranularitySuitableForSizing(Field[] ff) {
    ...
    for (Field f : ff) {
      if (f.isDate()) {
        continue;   // No date granularity use
      }
  ...
  public void writeAestheticScales(ElementStructure structure) {
    ...
    if (color == null && opacity == null && size.length == 0 && css.length == 0 && symbol == null) {
      return;
    }
  ...
  private void addColorScale(Param p, VisElement vis) {
    ...
    if (categories <= 0) {
      out.addChained("interpolate(d3.interpolateHcl)");   // Interpolate for numeric only
    }
  ...
  private void addSizeScale(String name, Param p, VisElement vis, String defaultTransform) {
    ...
    out.addChained("range([ ").add(Data.join(sizes, null, false)).add("])").endStatement();
  }
  ...
  private void defineCSSClassAesthetic(VisElement vis, boolean dataInside, Param p, String suffix) {
    ...
    if (field.preferCategorical()) {
      ...
      for (int i = 0; i < indices.length; i++) {
        indices[i] = Data.quote(cssPrefix + (i + 1));
      }
  ...
  private double getIncludeZeroFraction(ScalePurpose purpose, boolean domainWantsZeroIncluded) {
    ...
    if (purpose == ScalePurpose.sizeAesthetic) {
      return 1.0;        // Size always goes to zero
    }
    if (purpose == ScalePurpose.x) {
      return 0.1;                    // Do not want much empty space on x axis
    }
    if (domainWantsZeroIncluded) {
      return 1.0;
    }
    ...
    if (purpose.isCoord) {
      ...
      if (nBarArea == elements.length) {
        return 1.0;    // All bars?  Always go to zero
      }
      if (nBarArea > 0) {
        return 0.8;                   // Some bars?  Strongly want to go to zero
      }
    }
  ...
  public static Double[] getSizes(List<Param> params) {
    ...
    if (result.isEmpty()) {
      return new Double[]{MIN_SIZE_FACTOR, 1.0};
    }
    if (result.size() == 1) {
      result.add(0, MIN_SIZE_FACTOR);
    }
