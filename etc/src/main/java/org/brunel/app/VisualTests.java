...
import org.brunel.library.LibraryItem;
...
class VisualTests {
	...
	private final Map<String, LibraryItem> items;                       // Library items to build
	...
	private void run() throws Exception {
		...
		String[] libraryItems = text.substring(libStart, libEnd).split("\n");
		...
		for (String item : libraryItems) {
			...
			items.put(name, new LibraryItem(name, name, action, 1.0, fields));
		}
	...
	private void runTest(String test, int index) {
		...
		if (actionName.equals("-")) {
			...
			a = items.get(actionName).apply(fields);
