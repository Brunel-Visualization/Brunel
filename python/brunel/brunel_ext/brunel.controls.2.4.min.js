...
function BrunelEventHandlers(c){function f(){c.dataPreProcess(function(c){return c.filter(e.makeFilterStatement(c))})}var e;return{make_filter_handler:function(m){m||(m={});var a=m;e={filter:function(b,d,e){null==e&&(e=500);null!=b&&null!=d&&(a[b]=d,b=e,f(),c.rebuild(b))},makeFilterStatement:function(b){var d="",e;for(e in a)if(b===c.data(null,a[e].datasetIndex)){if("range"===a[e].filter_type)d+=e+" in "+a[e].filter.min+", "+a[e].filter.max;else if("category"===a[e].filter_type){var f;f=a[e].filter;
var g=e+" is ",j=void 0;for(j in f)g+=f[j]+", ";f=g.substring(0,g.length-1);d+=f}d+=";"}return d.substring(0,d.length-1)}};f();$.subscribe("filter."+c.visId,function(a,d,c,f){e.filter(d,c,f)})}}};var BrunelJQueryControlFactory=function(){function c(b){return null!=a[b]&&null!=a[b].id}function f(b){clearInterval(a[b].id);$("#"+a[b].buttonId).button("option","icons",{primary:"ui-icon-play"});delete a[b].id}function e(b,d,e,p,g,j){$("#"+p).button("option","icons",{primary:"ui-icon-pause"});a[b]||(a[b]={index:0});a[b].buttonId=p;g&&(a[b].index=0);c(b)&&f(b);a[b].id=setInterval(function(){a[b].index<e.length-1?($("#"+d).slider({values:[e[a[b].index],e[a[b].index+1]]}),a[b].index++):a[b].index=
0},j)}function m(a){jQuery.contains(document,$("#"+a)[0])?$("#"+a).SumoSelect():setTimeout(function(){m(a)},100)}var a={};return{make_range_slider:function(b,d,n,p,g,j,k,m,h,l){null==l&&(l=500);null==h&&(h=4);var q=BrunelData.auto_NumericExtentDetail.makeForField(k),r=BrunelData.auto_Auto.makeNumericScale(q,!0,[0,0],0,h+1,!0).divisions,v="slider"+b+n,s="animateButton"+b+n,q=r[0],t=r[r.length-1];h=$("<div />").addClass("horizontal-box");$("<text  />").addClass("title").html(p).appendTo(h);m&&$("<div />",
{id:s}).button({text:!1,icons:{primary:"ui-icon-play"}}).click(function(){var d=c(b)&&a[b].buttonId===s;c(b)?(f(b),d||e(b,v,r,s,!0,l)):e(b,v,r,s,!1,l)}).appendTo(h);g||(g=q);j||(j=t);var x=$("<div />").appendTo(h).addClass("legend control-label control-left-label");x.html(k.format(g));var w=$("<div />").addClass("legend control-label");w.html(k.format(j));p=2<t-q?1:(t-q)/r.length;g=$("<div />",{id:v}).slider({range:!0,min:q,max:t,step:p,values:[g,j],slide:function(a,b){x.html(k.format(b.values[0]));
w.html(k.format(b.values[1]))},change:function(a,c){$.publish("filter."+b,[n,{filter:{min:c.values[0],max:c.values[1]},filter_type:"range",datasetIndex:d}],0.9*l)}});g=$("<div />").append(g);h.append(g);w.appendTo(h);return h},make_category_filter:function(a,c,e,f,g,j){var k=$("<select />",{multiple:"multiple"}).addClass("select-filter"),u=k.uniqueId().attr("id"),h=$("<div />").addClass("horizontal-box");$("<text  />").addClass("title").html(f).addClass("control-title").appendTo(h);for(var l in g)f=
j&&0<=j.indexOf(g[l])?!0:!1,j||(f="selected"),$("<option />",{value:l,text:g[l],selected:f}).appendTo(k);k.appendTo(h);$("body").on("change","#"+u,function(){var f=[];$("#"+u+" option:selected").each(function(){f.push($(this).text())});$.publish("filter."+a,[e,{filter:f,filter_type:"category",datasetIndex:c}])});m(u);return h}}}();
