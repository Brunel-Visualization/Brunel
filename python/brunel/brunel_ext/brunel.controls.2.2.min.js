...
0},j)}function m(a){jQuery.contains(document,$("#"+a)[0])?$("#"+a).SumoSelect():setTimeout(function(){m(a)},100)}var a={};return{make_range_slider:function(b,d,n,p,g,j,k,m,h,l){null==l&&(l=500);null==h&&(h=4);var q=BrunelData.auto_Auto.makeNumericScale(k,!0,[0,0],0,h+1,!0).divisions,v="slider"+b+n,r="animateButton"+b+n,s=q[0],t=q[q.length-1];h=$("<div />").addClass("horizontal-box");$("<text  />").addClass("title").html(p).appendTo(h);m&&$("<div />",{id:r}).button({text:!1,icons:{primary:"ui-icon-play"}}).click(function(){var d=
c(b)&&a[b].buttonId===r;c(b)?(f(b),d||e(b,v,q,r,!0,l)):e(b,v,q,r,!1,l)}).appendTo(h);g||(g=s);j||(j=t);var x=$("<div />").appendTo(h).addClass("legend control-label control-left-label");x.html(k.format(g));var w=$("<div />").addClass("legend control-label");w.html(k.format(j));p=2<t-s?1:(t-s)/q.length;g=$("<div />",{id:v}).slider({range:!0,min:s,max:t,step:p,values:[g,j],slide:function(a,b){x.html(k.format(b.values[0]));w.html(k.format(b.values[1]))},change:function(a,c){$.publish("filter."+b,[n,
{filter:{min:c.values[0],max:c.values[1]},filter_type:"range",datasetIndex:d}],0.9*l)}});g=$("<div />").append(g);h.append(g);w.appendTo(h);return h},make_category_filter:function(a,c,e,f,g,j){var k=$("<select />",{multiple:"multiple"}).addClass("select-filter"),u=k.uniqueId().attr("id"),h=$("<div />").addClass("horizontal-box");$("<text  />").addClass("title").html(f).addClass("control-title").appendTo(h);for(var l in g)f=j&&0<=j.indexOf(g[l])?!0:!1,j||(f="selected"),$("<option />",{value:l,text:g[l],
selected:f}).appendTo(k);k.appendTo(h);$("body").on("change","#"+u,function(){var f=[];$("#"+u+" option:selected").each(function(){f.push($(this).text())});$.publish("filter."+a,[e,{filter:f,filter_type:"category",datasetIndex:c}])});m(u);return h}}}();
