...
function BrunelEventHandlers(c){function f(){c.dataPreProcess(function(c){return c.filter(e.makeFilterStatement())})}var e;return{make_filter_handler:function(m){m||(m={});var a=m;e={filter:function(b,d,e){null==e&&(e=500);null!=b&&null!=d&&(a[b]=d,b=e,f(),c.rebuild(b))},makeFilterStatement:function(){var b="",d;for(d in a){if("range"===a[d].filter_type)b+=d+" in "+a[d].filter.min+", "+a[d].filter.max;else if("category"===a[d].filter_type){var c;c=a[d].filter;var e=d+" is ",f=void 0;for(f in c)e+=
c[f]+", ";c=e.substring(0,e.length-1);b+=c}b+=";"}return b.substring(0,b.length-1)}};f();$.subscribe("filter."+c.visId,function(a,c,f,k){e.filter(c,f,k)})}}};var BrunelJQueryControlFactory=function(){function c(b){return null!=a[b]&&null!=a[b].id}function f(b){clearInterval(a[b].id);$("#"+a[b].buttonId).button("option","icons",{primary:"ui-icon-play"});delete a[b].id}function e(b,d,e,k,j,h){$("#"+k).button("option","icons",{primary:"ui-icon-pause"});a[b]||(a[b]={index:0});a[b].buttonId=k;j&&(a[b].index=0);c(b)&&f(b);a[b].id=setInterval(function(){a[b].index<e.length-1?($("#"+d).slider("values",0,e[a[b].index]),$("#"+d).slider("values",1,e[a[b].index+1]),
a[b].index++):a[b].index=0},h)}function m(a){jQuery.contains(document,$("#"+a)[0])?$("#"+a).SumoSelect():setTimeout(function(){m(a)},100)}var a={};return{make_range_slider:function(b,d,n,k,j,h,m,g,l){null==l&&(l=500);null==g&&(g=4);var p=BrunelData.auto_Auto.makeNumericScale(h,!0,[0,0],0,g+1,!0).divisions,s="slider"+b+d,q="animateButton"+b+d,t=p[0],u=p[p.length-1];g=$("<div />").addClass("horizontal-box");$("<text  />").addClass("title").html(n).appendTo(g);m&&$("<div />",{id:q}).button({text:!1,
icons:{primary:"ui-icon-play"}}).click(function(){var d=c(b)&&a[b].buttonId===q;c(b)?(f(b),d||e(b,s,p,q,!0,l)):e(b,s,p,q,!1,l)}).appendTo(g);k||(k=t);j||(j=u);var w=$("<div />").appendTo(g).addClass("legend control-label control-left-label");w.html(h.format(k));var v=$("<div />").addClass("legend control-label");v.html(h.format(j));n=$("<div />",{id:s}).slider({range:!0,min:t,max:u,step:Math.abs(u-t)/200,values:[k,j],slide:function(a,b){w.html(h.format(b.values[0]));v.html(h.format(b.values[1]))},
change:function(a,c){$.publish("filter."+b,[d,{filter:{min:c.values[0],max:c.values[1]},filter_type:"range"}],0.9*l)}});n=$("<div />").append(n);g.append(n);v.appendTo(g);return g},make_category_filter:function(a,c,e,f,j){var h=$("<select />",{multiple:"multiple"}).addClass("select-filter"),r=h.uniqueId().attr("id"),g=$("<div />").addClass("horizontal-box");$("<text  />").addClass("title").html(e).addClass("control-title").appendTo(g);for(var l in f)e=j&&0<=j.indexOf(f[l])?!0:!1,j||(e="selected"),
$("<option />",{value:l,text:f[l],selected:e}).appendTo(h);h.appendTo(g);$("body").on("change","#"+r,function(){var e=[];$("#"+r+" option:selected").each(function(){e.push($(this).text())});$.publish("filter."+a,[c,{filter:e,filter_type:"category"}])});m(r);return g}}}();
